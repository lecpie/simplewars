...
public class Controlleur {
	...
	public void partieGagnee(Joueur j) {
		JOptionPane.showMessageDialog(f, "<HTML> <FONT SIZE=200>" + j.getNom()
				+ " gagne !</HTML>", "Gagné", JOptionPane.INFORMATION_MESSAGE);
	}

	public boolean deplacer(Joueur j, Coordonnee org, Coordonnee dst) {
		Unite unit = map.getCellule(org).getUnit();

		// Si il n'y a pas d'unite a deplacer ou que l'unite selectionnee
		// n'appartient pas au joueur
		if (unit == null || unit.getJoueur() != j
				|| !respecteLimiteDeplacement(org, dst))
			return false;

		Cellule cellOrg = map.getCellule(org), cellDst = map.getCellule(dst);
	...
	public boolean respecteLimiteDeplacement(Coordonnee a, Coordonnee b) {
		...
		if (a.getX() == b.getX() && a.getY() != b.getY()) {
			// Alors il faudrait que Y1-Y2 soit plus petit que le ddéplacement
			// max.
	...
	private boolean pasObstaclex(Coordonnee a, Coordonnee b) {
		...
		int j = b.getY();
		for (int i = Math.min(a.getX(), b.getX()); i <= Math.max(a.getX(),
				b.getX()); i++) {
	...
	private boolean pasObstacley(Coordonnee a, Coordonnee b) {
		int i = a.getX();
		for (int j = Math.min(a.getY(), b.getY()); j <= Math.max(a.getY(),
				b.getY()); j++) {
			if (!map.getGrille()[i][j].getTerrain().isPraticable()) {
				return false;
			}
	...
	/**
	 * Fonction permettant de recuperer toutes les coordonnes de la grille
	 * accessible a partir d'une coordonne, pour un deplacement simple
	 * 
	 * @param a
	 * @return
	 */
	public void getRespectfullCases(Coordonnee a) {
		...
		for (int i = 0; i < map.getGrille().length; i++) {
			for (int j = 0; j < map.getGrille()[i].length; j++) {
				if (respecteLimiteDeplacement(a,
						map.getGrille()[i][j].getCoordonnee())) {
					if (map.getGrille()[i][j].getTerrain().isPraticable())
						listeCasesColoriees.add(map.getGrille()[i][j]
								.getCoordonnee());
				}
			}
		}
	...
	private void creerUnit(Joueur joueur, Unite unit) {
		try {
			...
			jop.showMessageDialog(null,
					"<HTML> <FONT SIZE=200><B>Plus de place</B></HTML>", "",
	...
	public void click(Coordonnee coordonnee) {
		...
		if (cellule.contientBatiment()
				&& cellule.getBatiment().getJoueur().equals(getJoueurCourant())) {
		...
		else if (cellule.contientUnite() && !deuxiemeClick  && cellule.getUnit().getJoueur() == getJoueurCourant()) {
			
			Unite unit = cellule.getUnit();
			unit.playSoundSelected();

			if (unit.isDejaDeplace())
				return;
		...
		} else if (deuxiemeClick
				&& !(cellule.contientUnite() && cellule.getUnit().getJoueur() == getJoueurCourant())) {
